...
func TestCollectToRealtimeAndEventsPipeline(t *testing.T) {
	...
	createPropertyTable(ctx, t, clickConn, table)
...
func TestPropertyBatchWriterToClickHouse(t *testing.T) {
	...
	propertyTableName := propertyTable.Physical
	createEventTable(ctx, t, clickConn, table.Physical)
	createPropertyTable(ctx, t, clickConn, table)
...
func createEventTable(ctx context.Context, t *testing.T, conn driver.Conn, tableName string) {
	...
	if err != nil {
		t.Fatalf("build clickhouse event DDL failed: %v", err)
	}
...
func createPropertyTable(ctx context.Context, t *testing.T, conn driver.Conn, table clickhouse.Table) {
	t.Helper()
	...
	if err != nil {
		t.Fatalf("build clickhouse property DDL failed: %v", err)
	}
	if err := conn.Exec(ctx, ddl); err != nil {
	...
	t.Cleanup(func() {
		dropCtx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
		defer cancel()
		_ = conn.Exec(dropCtx, "DROP TABLE IF EXISTS "+quoteIdent(propertyTable.Physical))
	})
}
