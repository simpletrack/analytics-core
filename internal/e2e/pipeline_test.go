...
func TestCollectToRealtimeAndEventsPipeline(t *testing.T) {
	...
	redisClient := openRedis(t, ctx)
	defer redisClient.Close()
	mysqlDB := openMySQL(t)
	clickConn := openClickHouseNative(t, ctx)
	...
	createEventTable(t, ctx, clickConn, table.Physical)
	...
	defer stopProcessor()
	processorDone := runProcessor(t, processorCtx, processor)
	...
	pageviews := waitForEvents(t, ctx, reader, storage.EventListQuery{
		TenantID:  key.TenantID,
		ProjectID: key.ProjectID,
		SourceID:  key.SourceID,
		EventName: "page_view",
	...
	events := waitForEvents(t, ctx, reader, storage.EventListQuery{
		TenantID:  key.TenantID,
		ProjectID: key.ProjectID,
		SourceID:  key.SourceID,
		EventName: "signup_clicked",
	...
	realtime := waitForRealtime(t, ctx, reader, storage.RealtimeQuery{
		TenantID:  key.TenantID,
		ProjectID: key.ProjectID,
		SourceID:  key.SourceID,
...
func openClickHouseNative(t *testing.T, ctx context.Context) driver.Conn {
	t.Helper()
...
func createEventTable(t *testing.T, ctx context.Context, conn driver.Conn, tableName string) {
	t.Helper()
...
func runProcessor(t *testing.T, ctx context.Context, processor *ingestion.Processor) <-chan error {
	t.Helper()
...
func waitForEvents(t *testing.T, ctx context.Context, reader storage.EventReader, query storage.EventListQuery) []storage.EventRecord {
	t.Helper()

	var records []storage.EventRecord
	waitForCondition(t, ctx, func() (bool, error) {
		var err error
...
func waitForRealtime(t *testing.T, ctx context.Context, reader storage.EventReader, query storage.RealtimeQuery) []storage.EventRecord {
	t.Helper()
...
func waitForCondition(t *testing.T, ctx context.Context, check func() (bool, error)) {
	t.Helper()
