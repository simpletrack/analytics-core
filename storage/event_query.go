...
const (
	...
	// EventFilterByVisitID filters by the canonical analytics visit key.
	EventFilterByVisitID EventFilterField = "visit_id"
...
// EventPropertyFilterEvidence describes one typed property predicate shape.
//
// It intentionally omits the predicate value. Query evidence is used for
// read-side planning and diagnostics, not for echoing potentially sensitive
// user or event property values back to callers.
type EventPropertyFilterEvidence struct {
	Scope     PropertyScope       // Scope separates event properties from user properties
	Name      string              // Name is the normalized property key used by the predicate
	ValueType PropertyValueType   // ValueType selects the typed property value column
	Operator  EventFilterOperator // Operator records the allowlisted comparison shape
}
...
type EventQueryEvidence struct {
	...
	PropertyFilters     []EventPropertyFilterEvidence // PropertyFilters records typed property predicate shapes without values
